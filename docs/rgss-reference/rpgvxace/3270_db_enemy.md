...
バトル中に表示される敵キャラの画像です。ダブルクリックすると開く［戦闘グラフィック］ウィンドウで画像ファイルを指定します。指定時、［色相］のスライダーを移動させると、グラフィックの色合いを変更できます。［（なし）］の場合、グラフィックを表示しません。
...
### ●行動パターン
 ...
 ![行動パターン](img/3270_02.png)

バトル中の敵の行動の仕方を設定します。初期値として通常攻撃が自動的に設定されます。ダブルクリックすると開く［戦闘行動］ウィンドウで、［スキル］に行動内容、［優先度］に採用の優先度、［行動条件］に採用条件を以下の基準をもとに指定します。戦闘中の行動は、後述の[“行動パターンの採用ルール”](#action_rule)にもとづいて決まります。
...
敵キャラの特徴です。設定欄の各行をダブルクリックと表示されるウィンドウで内容を定義します。詳細は[“特徴の設定方法”](3410_db_feature.md)を参照してください。
...
（1）設定した行動から条件を満たすものを拾い上げます。条件を満たす行動がひとつもない場合は行動を起こしません。

（2）条件を満たす行動のうち、最大の優先度をもつもの、最大優先度との差が2以内の優先度をもつものを採用候補とします。

（3）採用候補のうち、最大優先度との差が1と2の行動については、それぞれの採用確率を最大優先度の行動の3分の2、3分の1とします。同じ優先度の行動が複数あるとき、それぞれ採用確率は同じになります。
...
「6」が50％、「5」が33.3％（50％の3分の2）、「4」は16.6％（50％の3分の1）になります。「3」は候補になりません。

### 【例3】行動の優先度が「5」「3」「3」のとき


「5」が60％、「3」はそれぞれ20％（60％の3分の1）になります。

（4）候補になった行動の採用確率のもと、乱数で採用する行動を決定します。
